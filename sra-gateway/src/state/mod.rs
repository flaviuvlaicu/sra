...
use sra_network::{error::NetworkError, event::NarrowEvent, UniversalStream};
use sra_core::{
    agent::{
        ...
        loop {
            select! (
                ...
                Some(agent_types) = agent_types.next()=>{
                    let (uid, name, msg, peer_socket_addr): (Uuid, String, Result<sra_core::agent::EventOutBound, NetworkError>, std::net::SocketAddr) = agent_types;
                    ...
                    match message{
                        Some(InBound::EventRequest(
                            ...
                            if let Ok(client_token) = ClientToken::from_str(&token, &self.client_token) {
                                ...
                                if !publish_hosts.is_empty() {
                                    if let Some(cm_sender) = certificate_manager.as_ref() {
                                        let cert_required_connect = publish_hosts.iter().filter(|ph|matches!(ph.connect.protocol, sra_core::generic::Protocol::HTTP | sra_core::generic::Protocol::HTTPS | sra_core::generic::Protocol::QUIC));
                        ...
                        Some(InBound::DataRequest(
                            ...
                            if let Some(command) = command {
                                ...
                                let Some(connected_address) = connecting_address.as_ref().and_then(|addr|sra_core::generic::Connect::from_schemaed_string(addr)) else {
                        ...
                        Some(InBound::TlsTransparent(sni,mut stream,service_port))  =>{
                            if let Some(Ok((user_id,agent,connect))) = users.get_mut_agent_by_domain(&sni,service_port){ //todo
                                if connect.protocol == sra_core::generic::Protocol::TCP{
                                    let connection = Uuid::new_v4();
