    ...
    fn call(&mut self, req: Request<Body>) -> Self::Future {
        ...
        let handler = async move {
            ...
            if let Some(token) = req
                ...
                .map(|t| t.to_owned())
            {
                debug!("sra token found: {}", token);
                let Some(derived_key) = req
                    .headers()
                    .get(header::SEC_WEBSOCKET_KEY)
                    .map(|t| sra_network::ws::WsConnection::drive_key(t.as_bytes()))
                else {
                            ...
                            async move {
                                ...
                                match sender {
                                    Left(s) => {
                                        ...
                                        if let Ok(upgraded) = upgrade::on(req).await {
                                            let ws_connection = Box::new(
                                                sra_network::ws::WsConnectionBinary::from(
                                                    upgraded,
                                                )
                                                .await,
                                            );
                                    ...
                                    Right(s) => {
                                        ...
                                        if let Ok(upgraded) = upgrade::on(req).await {
                                            let ws_connection = Box::new(
                                                sra_network::ws::WsConnection::from(
                                                    upgraded,
                                                )
                                                .await,
                                            );
