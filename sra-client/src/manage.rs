...
impl ControlFactory {
    ...
    pub async fn connect(&mut self, quiet: bool) -> Result<RelayInfo, ClientError> {
        ...
        let task = tokio::spawn({
            ...
            async move {
                loop {
                    ...
                    match msg {
                        Ok(sra_core::client::EventInBound::ConnectionError(
                            connection_id,
                            msg,
                        )) => {
                        ...
                        Ok(sra_core::client::EventInBound::Peer2Peer(p2p)) => {
                            debug!("Peer2Peer: {:?}", p2p);
        ...
        let Some(sra_core::client::EventResponse::ActiveAgents(agents)) = request
        ...
        request
            .request(ClientEventOutBound::Request(
                ...
                ClientEventRequest::UpdateConstantSysInfo(
                    sra_core::client::ConstSystemInfo { local_addr },
                ),
            ))
            .await?;
...
impl From<&ArgCommands> for Instruction {
    fn from(cmd: &ArgCommands) -> Self {
        match cmd {
            ...
            ArgCommands::Shell(_) => unreachable!("Shell is handled before start()"),
