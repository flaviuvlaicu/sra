...
impl TransportFactory {
    ...
    pub async fn create_direct(
        ...
        let res = 'res: {
            let (socket, peer) = match sra_network::p2p::udp_punched_socket(
    ...
    async fn connect_direct(
        ...
        sra_network::p2p::Request::from(&connect)
            .write(&mut quic_socket)
            .await
            .map_err(|_| ClientError::UnableToCommunicateWithQuicBiStream)?;
        match sra_network::p2p::Response::read(&mut quic_socket)
            .await
            .map_err(|e| {
                if matches!(
                    e,
                    sra_network::error::NetworkError::P2PInvalidCommand
                ) {
            ...
            sra_network::p2p::Response::InvalidRequest => {
