...
fn main() -> Result<(), AgentError> {
    ...
    if args.daemon {
        ...
        let stdout = std::fs::File::create("/tmp/sra-agent.out")?;
        let stderr = std::fs::File::create("/tmp/sra-agent.err")?;
        let daemonize = Daemonize::new()
            .pid_file("/tmp/sra-agent.pid")
    ...
    loop {
        ...
        match event.next().await {
            ...
            Some(Ok(AgentEventInBound::Peer2Peer(p2p))) => {
                tokio::spawn({
                    async move {
                        let (socket, _) = match sra_network::p2p::udp_punched_socket(
                            (&p2p).into(),
                        ...
                        loop {
                            ...
                            let key = key.clone();
                            tokio::spawn(async move {
                                let Ok(r) = sra_network::p2p::Request::read(&mut s).await
                                else {
                                    ...
                                    && !policies.into_iter().any(|p| p.permit(&con))
                                {
                                    ...
                                    if sra_network::p2p::Response::write(
                                        &sra_network::p2p::Response::AccessDenied,
                                        &mut s,
                                    )
                                    .await
                                    .is_err()
                                ...
                                let Some(remote_addr) =
                                    ...
                                    if sra_network::p2p::Response::write(
                                        &sra_network::p2p::Response::UnableToResolve,
                                        &mut s,
                                    )
                                    .await
                                    .is_err()
                                ...
                                let stream = match socket {
                                    Ok(stream) => {
                                        if sra_network::p2p::Response::write(
                                            &sra_network::p2p::Response::Success,
                                            &mut s,
                                        )
                                        .await
                                        .is_err()
                                    ...
                                    Err(_e) => {
                                        ...
                                        if sra_network::p2p::Response::write(
                                            &sra_network::p2p::Response::Failed,
                                            &mut s,
                                        )
                                        .await
                                        .is_err()
            ...
            Some(Ok(sra_core::agent::EventInBound::Ping(ping_time))) => {
                let _ = event_sender.send(AgentEventOutBound::Pong(ping_time));
            }
            Some(Ok(sra_core::agent::EventInBound::Shutdown)) => {
